...
6. インデックスファイルの `dependencies` から該当IDを取り除いて保存する。
   - 削除対象IDが `dependencies` の値（配列内）に含まれる場合、そのIDを取り除く。
   - 削除対象IDが `dependencies` のキーに含まれる場合、そのエントリを削除する。
   - 空になったエントリは削除する。
7. 削除したタスクのIDの配列（子タスクのIDも含む）を出力する。
