...
| ------------------- | ----------- | -------------------------------------------------------------------------------------------------------- |
| `<id>`              | 必須        | 更新するタスクのID                                                                                       |
| `--<field> <value>` | 1つ以上必須 | 更新するfrontmatterフィールド名と値。`parent` と `dependencies` はindex.jsonで管理されるため指定できない |
...
| `parent`       | 指定不可。変更には `task move --parent <id>` を使用する          |
| `dependencies` | 指定不可。変更には `task dep add` / `task dep delete` を使用する |
...
6. `status` を `done` に変更する場合、インデックスファイルの `children[String(id)]` に含まれる子タスクを再帰的に `done` に更新する。
7. `status` を `doing` または `done` に変更する場合、以下のルールで親タスクを再帰的に更新する。この処理をルートに達するまで繰り返す。
   - 親タスクの全サブタスクが `done` であれば、親タスクを `done` に更新する
   - 親タスクの全サブタスクが `done` でなく、親タスクの `status` が `todo` であれば、親タスクを `doing` に更新する
8. 更新後のタスクを出力する。
...
| 禁止フィールドを指定した             | `cannot update "<field>": ...`    | `null`                                                      | `false`           |
