...
test("削除したタスクのIDの配列を返す", async () => {
  ...
  const { output } = await runCli(["delete", "1"], taskDir());
  expect(JSON.parse(output).result).toEqual({ ids: [1] });
});

test("タスクが存在しない場合はエラーをスローする", async () => {
  const { output, exitCode } = await runCli(["delete", "999"], taskDir());
...
test("インデックスファイルが存在しない場合はエラーにならない", async () => {
  ...
  const { output, exitCode } = await runCli(["delete", "1"], taskDir());
