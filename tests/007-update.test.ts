...
test("statusをdoneにすると子タスクも再帰的にdoneになる", async () => {
  ...
  expect(readFileSync(resolve(taskDir(), "3.md"), "utf-8")).toContain(
    "status: done",
  );
});

test("子タスクをdoingにすると親タスクがdoingになる", async () => {
  writeFileSync(
    resolve(taskDir(), "1.md"),
    "---\ntitle: 親タスク\nstatus: todo\n---\n",
  );
...
test("子タスクをdoneにすると親タスクがdoingになる", async () => {
  writeFileSync(
    resolve(taskDir(), "1.md"),
    "---\ntitle: 親タスク\nstatus: todo\n---\n",
  );
...
test("孫タスクをdoingにすると親・祖父タスクも再帰的にdoingになる", async () => {
  writeFileSync(
    resolve(taskDir(), "1.md"),
    "---\ntitle: 祖父タスク\nstatus: todo\n---\n",
  );
  ...
  await runCli(["update", "3", "--status", "doing"], taskDir());
  expect(readFileSync(resolve(taskDir(), "2.md"), "utf-8")).toContain(
    "status: doing",
  );
  expect(readFileSync(resolve(taskDir(), "1.md"), "utf-8")).toContain(
...
test("親タスクがすでにdoingの場合は変更しない", async () => {
  writeFileSync(
    resolve(taskDir(), "1.md"),
    "---\ntitle: 親タスク\nstatus: doing\n---\n",
  );
...
test("親タスクがすでにdoneの場合は変更しない", async () => {
  writeFileSync(
    resolve(taskDir(), "1.md"),
    "---\ntitle: 親タスク\nstatus: done\n---\n",
  );
